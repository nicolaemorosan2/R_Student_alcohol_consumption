...
#Intrebare 2: Influenteaza o serie de factori performantele academice?
view(elevi_varsta_relevanta)
...
#mate
mod_influenta_alcool_asupra_note_mate<-lm(g3_mate ~ walc + dalc + failures + studytime + traveltime + goout + higher + 
                                             famrel + freetime + absences + address + famsup + health + internet + romantic, data = elevi_varsta_relevanta)
summary(mod_influenta_alcool_asupra_note_mate)
tab_model(mod_influenta_alcool_asupra_note_mate)

#portugheza
mod_influenta_alcool_asupra_note_por<-lm(g3_por ~ walc + dalc + failures + studytime + traveltime + goout + higher + 
                                           famrel + freetime + absences + address + famsup + health + internet + romantic, data = elevi_varsta_relevanta)
summary(mod_influenta_alcool_asupra_note_por)
tab_model(mod_influenta_alcool_asupra_note_por)

model_por_bun<-lm(g3_por ~ dalc + failures + studytime + higher + address + health, data = elevi_varsta_relevanta)
summary(model_por_bun)
tab_model(model_por_bun)
#tot
...
#Intrebarea 2 - cum determina o serie de factori notele 
arbore<-rpart(g3_mate ~ walc + dalc + failures + studytime + traveltime + goout + higher + 
                famrel + freetime + absences + address + famsup + health + internet + romantic, data=avg_train)
rpart.plot(arbore) 
predictie<-predict(arbore, newdata = avg_test)
